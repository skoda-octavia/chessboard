...
export class BoardGenerator {
    ...
    generatePawns(color: PieceColor, row: number) {
        for (let i = 0; i < this.board.width; i++) {
            if (color == PieceColor.Black) { var pawn = new BlackPawn(row, i, this.board) }
            else { var pawn = new WhitePawn(row, i, this.board) }
            this.board.fields[row][i].setPiece(pawn);
            this.board.pawns.push(pawn)
        }
    }
    ...
    setBasePosition() {
        ...
        this.generatePawns(PieceColor.Black, 1)

        this.generatePawns(PieceColor.White, 6)
